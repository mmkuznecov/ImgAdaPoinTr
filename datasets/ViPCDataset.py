...
#                 print("(subset, dc['taxonomy_id'], s, i) " * 10, (subset, dc['taxonomy_id'], s, n_renderings))
                file_list.append({
                    ...
                    'img_path':
                    self.img_path % (dc['taxonomy_id'], s[:-3], random.randint(0,23)),
                })
    ...
    def __getitem__(self, idx):
        ...
        data = {}
        rand_idx = 0 #random.randint(0, self.n_renderings - 1) if self.subset=='train' else 0
...
#         assert data['gt'].shape[0] == self.npoints
        ...
        # load view 
        views = self.transform(Image.open(sample['img_path']))
        views = views[:3,:,:]
