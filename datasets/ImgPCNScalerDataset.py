...
class ImgPCNScaler(data.Dataset):
    ...
    def __init__(self, config, num_imgs_per_obj=6):
        ...
        # add for scaler
        with open('/home/jovyan/vchopuryan/PoinTr/scaler_dict.pkl', 'rb') as f:
        ...
        for k, v in res_dict.items():
            for model_id, model_v in v.items():
                if len(model_v) > 0:
                    filter_dict[k][model_id] = {}
                    filter_dict[k][model_id]['k'] = res_dict[k][model_id]['k']
                    filter_dict[k][model_id]['mode'] = res_dict[k][model_id]['mode']
        self.scaler_dict = filter_dict
#         print('self.scaler_dict', self.scaler_dict)
...
#         assert data['gt'].shape[0] == self.npoints
        ...
        try:
            scaler_coef = self.scaler_dict[sample['taxonomy_id']][sample['model_id']]['k']
            scaler_operator = (data['partial'] >= 0) * scaler_coef[1] + (data['partial'] < 0) * scaler_coef[0]
            data['partial'] = data['partial'] * scaler_operator
#             print(sample['taxonomy_id'], sample['model_id'])
        except KeyError:
            pass
#             print('nooooooooooo')
