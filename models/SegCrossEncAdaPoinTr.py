...
#         print('cls_vec', cls_vec.sum(-1))
        
        seg_emb = seg_emb.transpose(1, 2).transpose(0, 1)
        seg_emb = self.get_better_seg_size(seg_emb)
#         print('seg_emb', seg_emb.shape)
        x = x.transpose(0,1)
#         print('seg_emb + x', seg_emb.shape, x.shape)
        seg_emb *= cls_vec.sum(-1).expand(512, bs).unsqueeze(-1).expand(512, bs, 384)
        x = self.transformer_encoder3(torch.cat([x, seg_emb]))
        x = x[:512,:] + x[512:,:]
#         print('x', x.shape)                
        x = x.transpose(0,1)
#         print('x', x.shape) 
...
class SegCrossEncAdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
