...
class PCTransformer(nn.Module):
    ...
    def forward(self, xyz, img):
        ...
        x = self.transformer_encoder3(torch.cat([x, img_feat]))
        
        img_feat = x[256:,:]
        x = x[:256,:].transpose(0,1)
        #end img block
...
#             print(size_coarse_wo_denoise)
            ...
            return q, coarse, denoise_length, img_feat.mean(0)
            ...
            return q, coarse, 0, img_feat.mean(0)
        

######################################## PoinTr ########################################  

@MODELS.register_module()
class ImgResNetСrossEncAdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
    ...
    def forward(self, xyz, img):
        q, coarse_point_cloud, denoise_length, img_feat = self.base_model(xyz, img) # B M C and B M 3
...
#         print('B, M ,C', B, M ,C)
        ...
        global_feature = torch.max(global_feature, dim=1)[0] # B 1024
#         print('global_feature', global_feature.shape, img_feat.shape)
