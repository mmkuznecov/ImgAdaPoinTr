...
#         )
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.img_dim, nhead=6)
        self.transformer_encoder3 = nn.TransformerEncoder(encoder_layer, num_layers=3)
...
#               use_symeig_workaround=False,
            ...
            seg_emb *= cls_vec.sum(-1).expand(seg_emb.shape[0], bs).unsqueeze(-1).expand(seg_emb.shape[0], bs, 384)
...
#             print('q_transpose.shape', q.shape, q_temp.shape, torch.cat([q_temp, seg_emb]).shape)
            q_temp = self.transformer_encoder3(torch.cat([q_temp, seg_emb]))
            q_temp = q_temp[:512,:] + q_temp[512:,:]
...
#               use_symeig_workaround=False,
            ...
            q = q.transpose(0,1)
            q = self.transformer_encoder3(torch.cat([q, seg_emb]))
            q = q[:512,:] + q[512:,:]
            q = q.transpose(0,1)
            # end seg block
...
class SegCrossDecAdaPoinTr(nn.Module):
