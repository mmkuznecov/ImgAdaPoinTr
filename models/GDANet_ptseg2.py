...
from utils.GDANet_util import local_operator_withnorm, local_operator, GDM, SGCAM
...
class GDANet(nn.Module):
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        ###############
        """block 1"""
        x1 = local_operator_withnorm(x, norm_plt, k=30, device=self.device)
        x1 = F.relu(self.conv1(x1))
        x1 = F.relu(self.conv11(x1))
        x1 = x1.max(dim=-1, keepdim=False)[0]
        x1h, x1l = GDM(x1, M=512, device=self.device)
        ...
        x2 = local_operator(x1t, k=30, device=self.device)
        x2 = F.relu(self.conv2(x2))
        x2 = F.relu(self.conv21(x2))
        x2 = x2.max(dim=-1, keepdim=False)[0]
        x2h, x2l = GDM(x2, M=512, device=self.device)
        ...
        x3 = local_operator(x2t, k=30, device=self.device)
