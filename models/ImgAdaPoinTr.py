...
class ImgAdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.base_img_model = ModalityTransfer()
    ...
    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, denoised_coarse, denoised_fine, pred_fine, modality_pts = ret
        
        assert pred_fine.size(1) == gt.size(1)
#         print('modality_pts.shape', modality_pts.shape)
        # denoise loss
        ...
        # recon loss
#         loss_coarse_modality = self.loss_func(modality_pts, gt)
        loss_coarse = self.loss_func(pred_coarse, gt)
        loss_fine = self.loss_func(pred_fine, gt)
        loss_recon = loss_coarse + loss_fine #+ 0.1 * loss_coarse_modality
    ...
    def forward(self, xyz, img):
        for parameter in self.base_img_model.parameters():
            parameter.requires_grad = False
        xyz_from_img = self.base_img_model(img)
        xyz_stack = torch.hstack([xyz,  xyz_from_img])
        q, coarse_point_cloud, denoise_length = self.base_model(xyz_stack) # B M C and B M 3
...
#         print('B, M ,C', B, M ,C)
        ...
        if self.training:
            ...
            ret = (pred_coarse, denoised_coarse, denoised_fine, pred_fine, xyz_stack)
            return ret
