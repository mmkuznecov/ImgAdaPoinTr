...
def local_operator_withnorm(x, norm_plt, k, device):
    
    batch_size = x.size(0)
    num_points = x.size(2)
    
    x = x.view(batch_size, -1, num_points)
    norm_plt = norm_plt.contiguous().view(batch_size, -1, num_points)
    ...
    neighbor = neighbor.view(batch_size, num_points, k, num_dims)
#     print('neighbor_norm', neighbor_norm.shape, batch_size, num_points, k, num_dims, neighbor_norm)
