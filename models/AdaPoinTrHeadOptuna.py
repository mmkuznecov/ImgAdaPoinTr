...
class AdaPoinTrHeadOptuna(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
    ...
    def get_loss(self, ret, gt, epoch=1, alphas=None):
...
#         print('denoised_target', denoised_target3.shape, denoised_target2.shape, denoised_target1.shape)
        ...
        loss_denoised3 = self.loss_func(denoised_fine, denoised_target3)
        
        loss_denoised = loss_denoised1 * alphas['alpha_d1'] + loss_denoised2 * alphas['alpha_d2'] + loss_denoised3 * alphas['alpha_d3']
        ...
        loss_coarse = self.loss_func(pred_coarse, gt512)
        loss_coarse2 = self.loss_func(pred_coarse2, gt1024)
        loss_coarse3 = self.loss_func(pred_coarse3, gt4096)
        loss_fine = self.loss_func(pred_fine, gt)
        
        loss_recon = loss_coarse * alphas['alpha_c1'] + loss_coarse2 * alphas['alpha_c2'] + loss_coarse3 * alphas['alpha_c3'] + loss_fine
