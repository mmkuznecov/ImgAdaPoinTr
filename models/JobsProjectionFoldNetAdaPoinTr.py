...
FOCAL_LENGTH = 193.98969044771428
...
def extract_patches(images, coords, k):
    ...
    for b in range(B):
        ...
        for m in range(coords.shape[1]):
            ...
            y1, x1 = min(max(y - k, 0), 219), min(max(x - k, 0), 219)
            y2, x2 = max(min(y + k + 1, H), 5), max(min(x + k + 1, W), 5)  # +1 потому что верхняя граница не включена в срез
            if x2 <= x1 or y2 <= y1 <= 219:
                print(m, f'x1:x2  {x1}, {x2}', f'y1:y2   {y1} {y2}')

            patch = images[b, x1:x2, y1:y2, :]
            batch_patches.append(patch)

        patches.append(torch.stack(batch_patches))

    # Возвращает список тензоров патчей для каждого изображения в батче
    return torch.stack(patches)


@MODELS.register_module()
class ProjectionFoldNetAdaPoinTr(LightningModule):
    def __init__(self, config, **kwargs):
        super().__init__()
        ...
        self.reduce_map = nn.Linear(self.trans_dim + 1027 + 16, self.trans_dim)
...
#         self.img_encoder = ResNet()
        ...
        self.img_encoder = nn.Sequential(
            nn.Conv2d(3, 8, (3, 3), bias=False),
            nn.BatchNorm2d(8),
            nn.GELU(),
            nn.Conv2d(8, 16, (3, 3), bias=False),
            nn.BatchNorm2d(16),
            nn.GELU(),
        )
    ...
    def forward(self, xyz, img, rot_tensor, dist):
...
#         print('global_feature.unsqueeze(-2).expand(-1, M, -1).shape', global_feature.unsqueeze(-2).expand(-1, M, -1).shape)
        ...
        dist = dist.expand(M, B).transpose(0, 1)
        projection_cloud = coarse_point_cloud[:,:,[1, 0, 2]] * torch.tensor([-1, 1, 1]).cuda() # bs x 512 x 3
#         print('projection_cloud', projection_cloud.shape, coarse_point_cloud.shape)
#         print('rot_tensor && dist', rot_tensor.shape, dist.shape)
        projection_cloud = torch.bmm(projection_cloud, rot_tensor.transpose(1, 2))
#         print('projection_cloud', projection_cloud.shape)
        xx = torch.ceil((projection_cloud[:,:,0] * self.focalLength) / (projection_cloud[:,:,2] + dist)).int() + 112
        yy = torch.ceil((projection_cloud[:,:,1] * self.focalLength) / (projection_cloud[:,:,2] + dist)).int() + 112
        pxl_indxs = torch.stack([xx, yy], -1)
#         print('img && pxl_indxs', img.shape, pxl_indxs.shape)
        patches = extract_patches(img, pxl_indxs, k=2).transpose(2, 4)
#         print('pathes', patches.shape)
        patches = torch.stack([self.img_encoder(p) for p in patches]).reshape(B, M, -1)
#         print('pathes2', patches.shape)
...
#             img_feat.unsqueeze(-2).expand(-1, M, -1),
    ...
    def training_step(self, batch, batch_nb):
        partial = batch[0].cuda()
        gt = batch[1].cuda()
        img = batch[2].cuda()
        rot_tensor = batch[3].cuda()
        cam_dist = batch[4].cuda()
        
        ret = self(partial, img, rot_tensor, cam_dist)
        
        sparse_loss, dense_loss = self.module.get_loss(ret, gt, epoch)
        _loss = sparse_loss + dense_loss 
        _loss.backward()
        tensorboard_logs = {'train/coarse_loss_cd': sparse_loss,
                            'train/fine_loss_cd': dense_loss,
                            'train/all_loss_cd':  _loss,
                           }
        return {'loss': loss, 'many_loss': tensorboard_logs}
    
    def training_epoch_end(self, outputs):
        avg_coarse_loss = torch.stack([x['many_loss']['train/coarse_loss_cd'] for x in outputs]).mean()
        avg_fine_loss = torch.stack([x['many_loss']['train/fine_loss_cd'] for x in outputs]).mean()
        avg_all_loss = torch.stack([x['many_loss']['train/all_loss_cd'] for x in outputs]).mean()
        
        tensorboard_logs = {'train/coarse_loss_cd': avg_coarse_loss * 1000,
                            'train/fine_loss_cd': avg_fine_loss * 1000,
                            'train/all_loss_cd': avg_all_loss * 1000,
                           }
        return {'avg_train_loss': avg_all_loss, 'log': tensorboard_logs}
    ...
    def validation_step(self, batch, batch_nb):
        ...
        return {'val_L1': dense_loss_l1, 'val_L2': dense_loss_l2}
    
    def validation_epoch_end(self, outputs):
        avg_L1_loss = torch.stack([x['val_L1'] for x in outputs]).mean() * 1000
        avg_L2_loss = torch.stack([x['val_L2'] for x in outputs]).mean() * 1000
        
        tensorboard_logs = {'valid/CD_L1': avg_L1_loss, 'valid/CD_L2': avg_L2_loss}
