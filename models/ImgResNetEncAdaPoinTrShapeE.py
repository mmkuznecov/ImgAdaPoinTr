...
def extract_pxls(images, coords):
    ...
    for b in range(B):
        ...
        for m in range(coords.shape[1]):
            ...
            y, x = min(max(y, 0), 223), min(max(x, 0), 223)

            pxls = images[b, x, y, :]
            batch_patches.append(pxls)
...
class ImgResNetEncAdaPoinTrPointE(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        ...
        for param in self.upsampler_model.parameters():
            param.requires_grad = False

        self.upsampler_diffusion = diffusion_from_config(DIFFUSION_CONFIGS['upsample'])
        # for param in self.upsampler_diffusion.parameters():
        #     param.requires_grad = False
        ...
        self.sampler = PointCloudSampler(
            ...
            diffusions=[self.upsampler_diffusion],
...
#         print('B, M ,C', B, M ,C)
        ...
        # add projection part
        img2 = img.transpose(1,3).transpose(1,2)
...
#         print('projection_cloud', projection_cloud.shape)
        ...
        print('img img2', img.shape, img2.shape)
        pxls = extract_pxls(img2, pxl_indxs) #.transpose(2, 4)
        print('pxls', pxls.shape)
        coarse_point_cloud = torch.cat([coarse_point_cloud, pxls], dim=-1)
        print('coarse_point_cloud', coarse_point_cloud.shape)
        for x in tqdm(self.sampler.sample_batch_progressive(batch_size=B, model_kwargs=dict(
            images=img,
            low_res=coarse_point_cloud.transpose(1, 2)
            ))):
            samples = x
        # print('samples.shape', samples.shape)
        # samples = self.sampler.output_to_point_clouds(samples)
        # print('samples.shape2', len(samples), samples[0].shape, samples[-1].shape)
        ...
        if self.training:
            ...
            pred_fine = samples.reshape(B, -1, 3).contiguous()
