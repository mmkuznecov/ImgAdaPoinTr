...
#         print(x_out.shape)
        ...
        if self.training:
            ...
            q = self.layer_norm1(q_out + q) # b n c

            q_out, _ = self.self_attn3(q, q, q)
            q = self.layer_norm2(q_out + q)
            q_skip = q

            # layer 2: cross + self attention
            q_out, _ = self.cross_attn5(q , img_feat, img_feat)
            q = self.layer_norm3(q_out + q) # b n c

            q_out, _ = self.self_attn4(q, q, q)
            q = self.layer_norm4(q_out + q)

            q_out, _ = self.cross_attn6(q, q_skip, q_skip)
            q = self.layer_norm5(q_out + q)
            q = q.transpose(0,1)
            #end img block
...
class ImgResNetTrAdaPoinTr2(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
