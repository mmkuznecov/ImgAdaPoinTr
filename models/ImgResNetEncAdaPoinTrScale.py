...
class ImgResNetEncAdaPoinTrScale(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
    ...
    def forward(self, xyz, img, taxonomy_ids, model_ids):
        try:
            scaler_coef = self.scaler_dict[taxonomy_ids][model_ids]['k']
            scaler_operator = (xyz >= 0).int() * scaler_coef[1] + (xyz < 0).int() * scaler_coef[0]
            xyz = xyz * scaler_operator
            print(taxonomy_ids, model_ids)
        except KeyError:
            pass
