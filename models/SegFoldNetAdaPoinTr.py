...
class SegFoldNetAdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        ...
        self.reduce_map = nn.Linear(self.trans_dim + 1027 + 128, self.trans_dim)
...
#         print('B, M ,C', B, M ,C)
        ...
        norm_plt = points_normals.estimate_pointcloud_normals(coarse_point_cloud,
                                                             30,
                                                             disambiguate_directions=False,
#                                                              use_symeig_workaround=False,
                                                            )
        seg_emb, seg_idx = self.base_model.segmentator(coarse_point_cloud.transpose(1, 2), norm_plt, cls_vec)
#         seg_save_path = '/home/jovyan/vchopuryan/PoinTr/test_output/'
#         torch.save((coarse_point_cloud, seg_emb, seg_idx, img),
#                     f'{seg_save_path}{len(os.listdir(seg_save_path))}.pt')
        rebuild_feature = torch.cat([
            global_feature.unsqueeze(-2).expand(-1, M, -1),
            seg_emb.transpose(1, 2),
            q,                            # 64, 576, 384
            coarse_point_cloud], dim=-1)  # B M 1027 + C + 128
