...
class AdaPoinTrHeadEMD(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        ...
        if self.decoder_type == 'fold':
            ...
            if self.num_points is not None:
                ...
                self.decode_head = SimpleRebuildFCLayer(self.trans_dim * 2, step=self.num_points // self.num_query // 16)  # rebuild a cluster point
                assert self.num_points % self.num_query == 0
                self.decode_head2 = SimpleRebuildFCLayer((self.trans_dim + 3) * 2, step=self.num_points // self.num_query // 8)  # rebuild a cluster point 
                self.decode_head3 = SimpleRebuildFCLayer((self.trans_dim + 6) * 2, step=self.num_points // self.num_query // 8)  # rebuild a cluster point 
            else:
                self.factor = self.fold_step**2
    ...
    def build_loss_func(self):
        ...
        self.loss_func_coarse = emd.emdModule()

    def get_loss(self, ret, gt, epoch=1):
        out_pred, out_denoised = ret
        pred_coarse, pred_coarse2, pred_coarse3, pred_fine = out_pred
        denoised_coarse, denoised_coarse2, denoised_coarse3, denoised_fine = out_denoised
        
        assert pred_fine.size(1) == gt.size(1)

        # denoise loss
#         print('self.factor', self.factor)
        idx = knn_point(self.factor // 16, gt, denoised_coarse) # B n k 
        denoised_target = index_points(gt, idx) # B n k 3 
        denoised_target1 = denoised_target.reshape(gt.size(0), -1, 3)

        idx = knn_point(self.factor // 4, gt, denoised_coarse) # B n k 
        denoised_target = index_points(gt, idx) # B n k 3 
        denoised_target2 = denoised_target.reshape(gt.size(0), -1, 3)

        idx = knn_point(self.factor, gt, denoised_coarse) # B n k 
        denoised_target = index_points(gt, idx) # B n k 3 
        denoised_target3 = denoised_target.reshape(gt.size(0), -1, 3)
        
        
#         print('denoised_target', denoised_target3.shape, denoised_target2.shape, denoised_target1.shape)
        assert denoised_target1.size(1) == denoised_coarse2.size(1)
        assert denoised_target2.size(1) == denoised_coarse3.size(1)
        assert denoised_target3.size(1) == denoised_fine.size(1)
        loss_denoised1 = self.loss_func(denoised_coarse2, denoised_target1)
        loss_denoised2 = self.loss_func(denoised_coarse3, denoised_target2)
        loss_denoised3 = self.loss_func_coarse(denoised_fine, denoised_target , eps=0.005, iters=1003)
        
        loss_denoised = loss_denoised1 * 0.5 + loss_denoised2 * 0.5 + loss_denoised3 * 0.5

        # recon loss
        gt4096 = misc.fps(gt, 4096)
        gt1024 = misc.fps(gt4096, 1024)
        gt512 = misc.fps(gt1024, 512)
        
        loss_coarse = self.loss_func(pred_coarse, gt512, eps=0.005, iters=100)
        loss_coarse2 = self.loss_func_coarse(pred_coarse2, gt1024, eps=0.005, iters=100)
        loss_coarse3 = self.loss_func_coarse(pred_coarse3, gt4096, eps=0.005, iters=100)
        loss_fine = self.loss_func(pred_fine, gt)
        
        loss_recon = loss_coarse * 0.5 + loss_coarse2 * 0.5 + loss_coarse3 * 0.5 + loss_fine
        
        for_wandb = (loss_coarse.item(),
                     loss_coarse2.item(),
                     loss_coarse3.item(),
                     loss_fine.item(),
                     loss_denoised1.item(),
        ...
        return loss_denoised, loss_recon, for_wandb
...
#             print('rebuild_feature', rebuild_feature.shape)    # torch.Size([16, 576, 1411])
            rebuild_feature = self.reduce_map(rebuild_feature) # B M C 
#             print('rebuild_feature', rebuild_feature.shape)    # torch.Size([16, 576, 384])
            relative_xyz = self.decode_head(rebuild_feature)   # B M S 3
#             print('relative_xyz', relative_xyz.shape)          # torch.Size([16, 576, 2, 3]) 
            rebuild_points1 = (relative_xyz + coarse_point_cloud.unsqueeze(-2))  # B M S 3
#             print('rebuild_points1', rebuild_points1.shape)      # torch.Size([16, 576, 2, 3])
            
            # step 2
            rebuild_feature = torch.repeat_interleave(rebuild_feature, repeats=2, dim=2).reshape(B, M * 2, 384)
            rebuild_feature = torch.cat([rebuild_feature, rebuild_points1.reshape(B, -1, 3)], dim=-1)  # torch.Size([16, 576 * 2, 387])
#             print('rebuild_feature2', rebuild_feature.shape)
            
            relative_xyz = self.decode_head2(rebuild_feature)
#             print('relative_xyz2', relative_xyz.shape)
            rebuild_points2 = (relative_xyz + rebuild_points1.reshape(B, -1, 3).unsqueeze(-2))
#             print('relative_xyz2', relative_xyz.shape)
#             print('rebuild_points3', rebuild_points2.shape)
            
            # step 3
            rebuild_feature = torch.repeat_interleave(rebuild_feature, repeats=4, dim=2).reshape(B, M * 8, 387)
            rebuild_feature = torch.cat([rebuild_feature, rebuild_points2.reshape(B, -1, 3)], dim=-1)  # torch.Size([16, 576 * 2, 390])
#             print('rebuild_feature3', rebuild_feature.shape)
            relative_xyz = self.decode_head3(rebuild_feature)
            rebuild_points3 = (relative_xyz + rebuild_points2.reshape(B, -1, 3).unsqueeze(-2))
#             print('relative_xyz3', relative_xyz.shape)
#             print('rebuild_points3', rebuild_points3.shape)
            
            

        if self.training:
            # split the reconstruction and denoise task
            pred_fine = rebuild_points3[:, :-denoise_length * 8].reshape(B, -1, 3).contiguous()
            pred_coarse3 = rebuild_points2[:, :-denoise_length * 2].reshape(B, -1, 3).contiguous()
            pred_coarse2 = rebuild_points1[:, :-denoise_length].reshape(B, -1, 3).contiguous()
            pred_coarse = coarse_point_cloud[:, :-denoise_length].contiguous()
#             print('rebuild shapes:', pred_fine.shape, pred_coarse3.shape, pred_coarse2.shape, pred_coarse.shape)
            
            denoised_fine = rebuild_points3[:, -denoise_length * 8:].reshape(B, -1, 3).contiguous()
            denoised_coarse3 = rebuild_points2[:, -denoise_length * 2:].reshape(B, -1, 3).contiguous()
            denoised_coarse2 = rebuild_points1[:, -denoise_length:].reshape(B, -1, 3).contiguous()
            denoised_coarse = coarse_point_cloud[:, -denoise_length:].contiguous()
#             print('denoised shapes:', denoised_fine.shape, denoised_coarse3.shape, denoised_coarse2.shape, denoised_coarse.shape)
            
            assert pred_fine.size(1) == self.num_query * self.factor
            assert pred_coarse.size(1) == self.num_query

            out_pred = (pred_coarse, pred_coarse2, pred_coarse3, pred_fine)
            out_denoised = (denoised_coarse, denoised_coarse2, denoised_coarse3, denoised_fine) 
            return (out_pred, out_denoised)

        else:
            assert denoise_length == 0
            rebuild_points = rebuild_points3.reshape(B, -1, 3).contiguous()  # B N 3
#             print(rebuild_points.shape, rebuild_points3.shape)
