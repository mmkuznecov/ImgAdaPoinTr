...
class SPD(nn.Module):
    ...
    def forward(self, pcd_prev, feat_global=None, K_prev=None):
        ...
        print('feat_1_max', torch.max(feat_1, 2, keepdim=True)[0].repeat((1, 1, feat_1.size(2))).shape)
        print('feat_global', feat_global.repeat(1, 1, feat_1.size(2)).shape, self.global_feat)
...
#         print('feat_1', feat_1.shape)
#         print('feat_1_max', torch.max(feat_1, 2, keepdim=True)[0].repeat((1, 1, feat_1.size(2))).shape)
#         print('feat_global', feat_global.shape, self.global_feat)
        if feat_global.shape[-1] != 1:
            feat_1 = torch.cat([feat_1,
                                torch.max(feat_1, 2, keepdim=True)[0].repeat((1, 1, feat_1.size(2))),
                                feat_global], 1)
        else:
...
class AdaPoinTrNewRebuildHead(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        ...
        self.reduce_map = nn.Linear(self.trans_dim + 1027, self.trans_dim + 128) # 384 +128
        ...
        self.feat_ups1 = nn.Upsample(scale_factor=up_factors[0])
        self.feat_ups2 = nn.Upsample(scale_factor=up_factors[1])
        # End SPD from SnowFlake
    ...
    def get_loss(self, ret, gt, epoch=1):
        (p_0, p_1, p_2, pred_fine), denoised_coarse = ret
        
        assert pred_fine.size(1) == gt.size(1)
        
        # denoise loss
        idx = knn_point(1, gt, denoised_coarse) # B n k 
        denoised_target = index_points(gt, idx) # B n k 3 
        denoised_target = denoised_target.reshape(gt.size(0), -1, 3)
        assert denoised_target.size(1) == denoised_coarse.size(1)
        loss_denoised = self.loss_func(denoised_coarse, denoised_target)
        loss_denoised = loss_denoised * 0.5
        
        idx = knn_point(1, gt, p_0) # B n k 
        p_0_target = index_points(gt, idx) # B n k 3 
        p_0_target = p_0.reshape(gt.size(0), -1, 3)
        
        idx = knn_point(1, gt, p_1) # B n k 
        p_1_target = index_points(gt, idx) # B n k 3 
        p_1_target = p_1.reshape(gt.size(0), -1, 3)
        
        idx = knn_point(1, gt, p_2) # B n k 
        p_2_target = index_points(gt, idx) # B n k 3 
        p_2_target = p_2.reshape(gt.size(0), -1, 3)
...
#         print(p_0, p_0_target)
#         print(p_1, p_1_target)
#         print(p_2, p_2_target)
#         print(pred_fine)

        # recon loss
        loss_p_0 = self.loss_func(p_0, p_0_target)
        loss_p_1 = self.loss_func(p_1, p_1_target)
        loss_p_2 = self.loss_func(p_2, p_2_target)
        loss_fine = self.loss_func(pred_fine, gt)
        print(loss_denoised, loss_p_0, loss_p_1, loss_p_2, loss_fine)
        loss_recon = loss_p_0 * 0.5 + loss_p_1 * 0.5 + loss_p_2  * 0.5 + loss_fine
        # * self.alpha_loss[epoch]
    ...
    def forward(self, xyz):
        ...
        global_feature = torch.max(global_feature, dim=1)[0] # B 1024
#         print(q.shape, coarse_point_cloud.shape)
        rebuild_feature = torch.cat([
            global_feature.unsqueeze(-2).expand(-1, M - denoise_length, -1),
            q[:,:-denoise_length,:],
            coarse_point_cloud[:,:-denoise_length,:]], dim=-1)  # B M 1027 + C
        
#         print('rebuild_feature', rebuild_feature.shape)
        rebuild_feature = self.reduce_map(rebuild_feature).permute(0, 2, 1).contiguous()
#         print('rebuild_feature', rebuild_feature.shape)
        out = []
#         print('coarse_point_cloud', coarse_point_cloud.shape)
        out.append(coarse_point_cloud[:, :-denoise_length, :])
        K_prev = None
        denoise_pcd = coarse_point_cloud[:, -denoise_length:].contiguous()
#         print('denoise_pcd', denoise_pcd.shape)
        coarse_point_cloud = coarse_point_cloud.permute(0, 2, 1)[:, :,:-denoise_length].contiguous()
#         print('coarse_point_cloud.permute(0, 2, 1)', coarse_point_cloud.shape)
        for ind, upper in enumerate(self.uppers):
#             pcd, K_prev = upper(coarse_point_cloud, rebuild_feature, K_prev)
            if ind == 1:
                rebuild_feature = self.feat_ups1(rebuild_feature)
            elif ind == 2:
                rebuild_feature = self.feat_ups2(rebuild_feature)
            coarse_point_cloud, K_prev = upper(coarse_point_cloud, rebuild_feature, K_prev)
#             print('coarse_point_cloud', coarse_point_cloud.shape)
            out.append(coarse_point_cloud.permute(0, 2, 1).contiguous())

#         return arr_pcd
        if self.training:
            return out, denoise_pcd
        else:
            return out[0], out[-1]   
